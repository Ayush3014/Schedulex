...
app.post("/feedback",(req,res)=>{
  ...
  connection.query("CREATE TABLE if not exists Feedback_Form(Feedback_ID int AUTO_INCREMENT primary key,Name varchar(1000),Email varchar(1000),Description varchar(1000))",
  function (err, result) {
    if (err) throw err;
    
   var values=[[null,req.body.name,req.body.email,req.body.description]];
    
    var sql = "INSERT INTO Feedback_Form VALUES ?";
    connection.query(sql, [values], function (err, result) {
      if (err) throw err;
      else {
        console.log("Data inserted Successfully");
